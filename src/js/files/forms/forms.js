...
export let formValidate = {
	...
	validateInput(formRequiredItem) {
      ...
      if (formRequiredItem.tagName !== 'SELECT') {
        ...
        if (formRequiredItem.parentElement.offsetHeight > 0) {
          if (!formRequiredItem.value.trim()) {
            this.addError(formRequiredItem);
            error++;
          } else {
	...
	document.addEventListener("click", function (e) {
		...
		if (targetElement.closest('.quantity__button')) {
			...
			if (targetElement.classList.contains('quantity__button_plus')) {
				value++;
        if (targetElement.closest('.quantity').querySelectorAll('._disabled').length) {
          targetElement.closest('.quantity').querySelectorAll('._disabled').forEach(e=>e.classList.remove('_disabled'))
        }
			} else {
				--value;
				if (value < 1) {
